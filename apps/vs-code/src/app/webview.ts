...
import { join, relative, isAbsolute, parse as parsePath } from 'path'
...
export interface WebviewOptions extends PluginConnectorOptions {
  ...
  relativeTo?: 'extension' | 'workspace'
  column?: ViewColumn
  devMode?: boolean
}
...
export class WebviewPlugin extends PluginConnector {
  ...
  getPath(link: string = '') {
    const baseUrl = join(this.options.context.extensionPath, this.profile.url);
    const path = relative(baseUrl, link);
